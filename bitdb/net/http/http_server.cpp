...
#include "bitdb/common/file_util.h"
...
#include "bitdb/net/http/http_server.h"
...
co::Task<> HttpServer::OnRequest(TcpConnectionPtr conn, TcpServer& server) {
  for (;;) {
    ...
    auto timer_id =
        Singleton<timer::TimerManager>::Get()->AddTimer(1000, [=] {
    ...
    Singleton<timer::TimerManager>::Get()->RemoveTimer(timer_id);
